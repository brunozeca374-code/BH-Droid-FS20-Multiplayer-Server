...
const BRIDGE_VERSION = 'V23_LATEST_WINS_LOW_LATENCY_FS20';
const WS_PATH = '/relay';
...
function noteRealtimeDrop(live, guest, direction, pending, limit, reason) {
  if (!live) return;
  ...
  if (drops <= 5 || drops % 250 === 0) {
    console.warn(`[BHFS20/WSS] realtime ${reason} room=${live.roomId} dir=${direction} stream=${guest ? guest.streamId : 0} pending=${pending} limit=${limit} drops=${drops}`);
  }
}

function ensureRealtimeState(ws) {
  if (!ws.bhRealtimePending) ws.bhRealtimePending = new Map();
  if (typeof ws.bhRealtimeSending !== 'boolean') ws.bhRealtimeSending = false;
}

function scheduleRealtimeFlush(ws) {
  if (!ws || ws.readyState !== WebSocket.OPEN || ws.bhRealtimeFlushTimer) return;
  ws.bhRealtimeFlushTimer = setTimeout(() => {
    ws.bhRealtimeFlushTimer = null;
    flushRealtimePending(ws);
  }, 1);
  if (ws.bhRealtimeFlushTimer && typeof ws.bhRealtimeFlushTimer.unref === 'function') {
    ws.bhRealtimeFlushTimer.unref();
  }
}

function sendRealtimeNow(ws, entry) {
  if (!ws || ws.readyState !== WebSocket.OPEN || !entry) return false;
  ensureRealtimeState(ws);
  if (ws.bhRealtimeSending) return false;

  ws.bhRealtimeSending = true;
  try {
    ws.send(entry.data, entry.options, error => {
      ws.bhRealtimeSending = false;
      if (error) {
        console.error(`[BHFS20/WSS] realtime send failed: ${error.message}`);
      }
      flushRealtimePending(ws);
    });
    return true;
    ...
    ws.bhRealtimeSending = false;
    console.error(`[BHFS20/WSS] realtime send threw: ${error.message}`);
    scheduleRealtimeFlush(ws);
    return false;
  }
}

function flushRealtimePending(ws) {
  if (!ws || ws.readyState !== WebSocket.OPEN) return false;
  ensureRealtimeState(ws);
  if (ws.bhRealtimeSending || ws.bhRealtimePending.size === 0) return false;

  // Do not append the newest gameplay frame behind bytes already waiting in the
  // TCP/WebSocket writer. Wait for the current write to clear, then forward the
  // newest ENet datagram we retained for that stream.
  if (wsPendingBytes(ws) > 0) {
    scheduleRealtimeFlush(ws);
    return false;
  }

  const iterator = ws.bhRealtimePending.entries().next();
  if (iterator.done) return false;
  const [key, entry] = iterator.value;
  ws.bhRealtimePending.delete(key);
  return sendRealtimeNow(ws, entry);
}
...
function wsRealtimeSend(ws, data, options, live, guest, direction) {
  ...
  const ageMs = Date.now() - Number((guest && guest.joinedAt) || 0);
  const inStartupGrace = ageMs >= 0 && ageMs < RELAY_STARTUP_GRACE_MS;

  // During world sync we keep the older permissive behavior so reliable ENet
  // bootstrap traffic is not aggressively coalesced.
  if (inStartupGrace) {
    if (pending > limit) {
      noteRealtimeDrop(live, guest, direction, pending, limit, 'startup-drop');
      return false;
    }
    ws.send(data, options);
    return true;
  ...
  ensureRealtimeState(ws);
  const streamId = Number((guest && guest.streamId) || 0);
  const key = `${direction}:${streamId}`;

  // IMPORTANT: the previous code dropped the NEW packet when backpressure was
  // detected. That preserved stale steering/throttle packets already queued in
  // TCP and discarded the driver's latest command. Post-sync we invert that:
  // keep at most one pending datagram per stream and replace it with the newest.
  // ENet reliable traffic can retransmit exactly as it would over lossy UDP.
  if (ws.bhRealtimeSending || pending > 0) {
    const replacing = ws.bhRealtimePending.has(key);
    ws.bhRealtimePending.set(key, {
    ...
    if (replacing) {
      noteRealtimeDrop(live, guest, direction, pending, limit, 'coalesced-stale');
    }
    scheduleRealtimeFlush(ws);
    return true;
  }

  return sendRealtimeNow(ws, {
...
wss.on('connection', (ws) => {
  ...
  // Flush the newest coalesced gameplay frame as soon as Node reports that the
  // TCP writer drained. The 1 ms fallback timer in scheduleRealtimeFlush covers
  // cases where no explicit drain event is emitted.
  try {
    if (ws._socket) ws._socket.on('drain', () => flushRealtimePending(ws));
  } catch (_) {}
  ...
  ws.on('close', () => {
    if (ws.bhRealtimeFlushTimer) { clearTimeout(ws.bhRealtimeFlushTimer); ws.bhRealtimeFlushTimer = null; }
    if (ws.bhRealtimePending) ws.bhRealtimePending.clear();
