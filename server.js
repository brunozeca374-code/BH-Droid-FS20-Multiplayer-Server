...
const BRIDGE_VERSION = 'V23_LOW_LATENCY_BACKPRESSURE_FS20';
const WS_PATH = '/relay';
...
const RELAY_MAX_BUFFERED_BYTES = Math.max(16 * 1024, Number(process.env.RELAY_MAX_BUFFERED_BYTES || 48 * 1024));
...
function relayQueueLimit(guest) {
  const ageMs = Date.now() - Number((guest && guest.joinedAt) || 0);
  return ageMs >= 0 && ageMs < RELAY_STARTUP_GRACE_MS
    ? RELAY_STARTUP_MAX_BUFFERED_BYTES
...
function wsRealtimeSend(ws, data, options, live, guest, direction) {
  ...
  const limit = relayQueueLimit(guest);
  if (live) {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/health')) {
    ...
    const body = JSON.stringify({
      ...
      relayMaxBufferedBytes: RELAY_MAX_BUFFERED_BYTES,
