...
const TEST_LOG_VERSION = "FS20_TEST_LOG_V2";
const MAX_BODY_CAPTURE_BYTES = Number.parseInt(process.env.MAX_BODY_CAPTURE_BYTES || "65536", 10);
const MAX_LOG_PREVIEW_CHARS = Number.parseInt(process.env.MAX_LOG_PREVIEW_CHARS || "2048", 10);
const rooms = new Map();
...
function testLog(kind, data = {}) {
  try {
    console.log(`[${TEST_LOG_VERSION}] ${nowIso()} ${kind} ${JSON.stringify(data)}`);
  } catch (_) {
    console.log(`[${TEST_LOG_VERSION}] ${nowIso()} ${kind}`);
  }
}
...
function bufferSummary(buffer, totalBytes = null) {
  const b = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || "");
  const text = b.toString("utf8")
    .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, ".");
  return {
    bytes: totalBytes == null ? b.length : totalBytes,
    capturedBytes: b.length,
    textPreview: text.slice(0, MAX_LOG_PREVIEW_CHARS),
    hexPreview: b.subarray(0, 256).toString("hex")
  };
}
...
function send(res, status, type, body) {
  ...
  const traceId = res.__fs20TraceId || "";
  testLog("HTTP_RESPONSE_BODY", {
    traceId,
    status,
    contentType: type,
    ...bufferSummary(b)
  ...
  res.writeHead(status, {
    ...
    "x-fs20-diagnostic": TEST_LOG_VERSION,
    "x-fs20-trace": traceId
  });
  res.end(b);
}
...
function handleHttpRequest(req, res, url, startedAt, capturedBody, totalBodyBytes) {
  const traceId = req.__fs20TraceId || "";
  ...
  testLog("HTTP_BODY", {
    ...
    ...bufferSummary(capturedBody, totalBodyBytes)
  });
  ...
  // Accept GET and POST on the diagnostic bridge. POST data is captured above
  // so we can see whether the client changes transport after the first request.
  if ((req.method === "GET" || req.method === "POST") && url.pathname === "/b") {
    url.searchParams.set("action", "boot");
    return bridge(req, res, url);
  }

  if ((req.method === "GET" || req.method === "POST") && url.pathname === "/bridge") {
    return bridge(req, res, url);
  }

  if (req.method === "HEAD" && (url.pathname === "/bridge" || url.pathname === "/b")) {
    testLog("BRIDGE_HEAD", { traceId, path: url.pathname });
    res.writeHead(200, {
  ...
  if (req.method === "GET" && url.pathname === "/") {
    return sendJson(res, 200, {
      ...
      mode: "custom-http-master-directory-v3-diagnostic",
      api: API_PREFIX,
      bridge: "/bridge",
      activeRooms: rooms.size,
      time: nowIso(),
      testLog: TEST_LOG_VERSION
    });
  }
...
const server = http.createServer((req, res) => {
  ...
  req.__fs20TraceId = traceId;
  res.__fs20TraceId = traceId;
  ...
  req.on("data", (chunk) => {
    ...
    if (capturedBodyBytes < MAX_BODY_CAPTURE_BYTES) {
      const remaining = MAX_BODY_CAPTURE_BYTES - capturedBodyBytes;
      const piece = chunk.length > remaining ? chunk.subarray(0, remaining) : chunk;
      chunks.push(piece);
      capturedBodyBytes += piece.length;
    }
  });

  req.on("aborted", () => {
    testLog("HTTP_ABORTED", {
      traceId,
      method: req.method,
      path: url.pathname,
      totalBodyBytes
  ...
  req.on("end", () => {
    const capturedBody = Buffer.concat(chunks);
    handleHttpRequest(req, res, url, startedAt, capturedBody, totalBodyBytes);
  });
});
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`[FS20] V3 diagnostic bridge listening on 0.0.0.0:${PORT}`);
  testLog("TEST_LOG_READY", {
    port: PORT,
    server: SERVER_NAME,
    mode: TEST_LOG_VERSION
  });
});
