...
const BRIDGE_VERSION = 'V18_WSS_FS20';
const WS_PATH = '/relay';
...
function closeRelayGuest(ws, reason = 'guest disconnected') {
  ...
  saveRooms();
  console.log(`[BHFS20/WSS] guest left room=${roomId} stream=${streamId}`);
}
...
function handleRelayText(ws, text) {
  ...
  if (msg.type === 'join') {
    ...
    live.guests.set(streamId, {
      ws, streamId, installId: String(msg.installId || '').slice(0, 160), joinedAt: Date.now()
    });
    ws.bhRole = 'guest';
    ws.bhRoomId = roomId;
    ...
    wsSendJson(live.host, {
      ...
      installId: String(msg.installId || '').slice(0, 160)
    });
...
async function handleBridge(req, res, url) {
  ...
  if (action === 'remove') {
    ...
    closeRelayRoom(String(id), 'room removed', true);
    rooms.delete(id);
    saveRooms();
